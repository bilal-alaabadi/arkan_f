...
const Navbar = () => {
    ...
    return (
        <header className="w-full bg-[#f5f5f5] shadow-sm relative z-50 pt-10 ">
            <div className=" mx-auto px-4 ">
                ...
                <div className="md:hidden flex items-center justify-between h-16 mb-2 pb-12 pt-4">
                    ...
                    {/* الجانب الأيمن - تسجيل الدخول/الحساب + السلة */}
                    <div className="flex items-center gap-4 "  >
                        {user ? (
                            ...
                            <Link 
                                to="/login" 
                                className="text-[#d3ae27] text-2xl"
                            >
                ...
                <div className="hidden md:flex items-center justify-between h-20 pb-7">
                    ...
                    <div className="flex items-center gap-4">
                        {user ? (
                            ...
                            <Link 
                                to="/login" 
                                className="text-[#d3ae27] hover:text-[#d3ae27] text-3xl transition-colors"
                            >
                                <i className="ri-user-line"></i>
                            </Link>
                        )}
                ...
                <nav className="hidden md:flex justify-center border-t border-gray-700 py-4 mt-2 ">
                    <div className="flex gap-10">
                        <Link to="/shop" className="text-[##1D1D1D ] hover:text-[#d3ae27] font-bold text-xl transition-colors">
                            المنتجات
                        </Link>
                        <Link to="/" className="text-[##1D1D1D ] hover:text-[#d3ae27] font-bold text-xl transition-colors">
                            الصفحة الرئيسية
                        </Link>
                        <Link to="/about" className="text-[##1D1D1D ] hover:text-[#d3ae27] font-bold text-xl transition-colors">
                            من نحن
                        </Link>
                    </div>
                </nav>
            ...
            {/* قائمة الهاتف المنزلقة من الأسفل */}
                ...
                <div className={`absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-xl transition-transform duration-300 ${isMobileMenuOpen ? 'translate-y-0' : 'translate-y-full'}`}>
                    <div className="px-6 py-6 flex flex-col items-center gap-4">
                        ...
                        <Link 
                            to="/shop" 
                            onClick={handleMobileMenuToggle}
                            className="w-full text-center py-4 px-6 font-medium text-xl text-[##1D1D1D ] hover:bg-[#f8f8f8] rounded-lg transition-all duration-300"
                        >
                            المنتجات
                        </Link>
                        <Link 
